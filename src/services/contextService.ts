...
class ContextService {
	...
	private loadContext(): UserContext {
		...
		if (stored) {
			try {
				...
				console.warn(
					"[ContextService] Failed to parse stored context, creating new"
				);
			}
		}
	...
	private recordPageDuration(): void {
		if (
			this.currentPage &&
			this.pageEnterTime &&
			this.context.pageVisits.length > 0
		) {
			const lastVisit =
				this.context.pageVisits[this.context.pageVisits.length - 1];
	...
	private scrapeAllPages(): void {
		...
		setTimeout(() => {
			this.scrapeSection(
				"Home",
				"The DoGood app helps you coordinate service, productivity, and self-improvement activities. Earn XP by completing tasks and challenges. Access the DoGood Companion voice assistant for guidance and support."
			);
			this.scrapeSection(
				"Serve",
				"Service section: Find and participate in community service opportunities. Contribute to causes you care about and make a positive impact."
			);
			this.scrapeSection(
				"Productivity",
				"Productivity section: Track your tasks, set goals, and improve your efficiency. Earn XP by completing productivity challenges."
			);
			this.scrapeSection(
				"Self-improve",
				"Self-improvement section: Work on personal growth through various activities and challenges. Build better habits and develop new skills."
			);
			this.scrapeSection(
				"Shop",
				"Shop section: Spend your earned XP on rewards and items. Redeem points for real-world benefits and digital perks."
			);
			this.scrapeSection(
				"Stats",
				"Stats section: View your progress, achievements, and XP history. Track your journey across all three pillars."
			);

			console.log(
				"[ContextService] Website content scraped:",
				this.context.websiteContent
			);
		}, 500);
	}
	...
	logActivity(
		...
		console.log(
			`[ContextService] Activity: ${type} - ${description}`,
			metadata
		);
	}
	 ...
	 */
	updatePreferences(
		preferences: Partial<UserContext["userPreferences"]>
	): void {
	...
	startTask(taskId: string, taskTitle: string): void {
		if (!this.context.tasksInProgress.includes(taskId)) {
			...
			this.logActivity("task_started", `Started task: ${taskTitle}`, {
				taskId,
			});
		}
	}
	...
	completeTask(taskId: string, taskTitle: string, xpGained: number): void {
		this.context.tasksInProgress = this.context.tasksInProgress.filter(
			(id) => id !== taskId
		);
	...
	/**
	 * Log Claude orchestration event
	 */
	logOrchestration(intent: string, commands: any): void {
		this.logActivity(
			"orchestration",
			`Claude orchestrated: ${intent}`,
			commands
		);
	}

	/**
	 * Get formatted transcript for Claude orchestration
	 */
	getTranscriptForClaude(): string {
		const recentActivities = this.context.activities.slice(-10);

		if (recentActivities.length === 0) {
			return "No recent conversation history.";
		}

		let transcript = "Recent conversation and activities:\n";
		recentActivities.forEach((activity) => {
			const timeAgo = this.formatTimeAgo(activity.timestamp);
			transcript += `[${timeAgo}] ${activity.description}\n`;
		});
	...
	async syncToBackend(): Promise<boolean> {
		try {
			const response = await fetch(
				`/api/context?sessionId=${this.context.sessionId}`,
				{
					method: "POST",
